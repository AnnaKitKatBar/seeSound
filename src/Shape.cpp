...
void Shape::loadMesh(const string &meshName)
{
	...
	if(!rc) {
		...
		posBuf = shapes[0].mesh.positions;  //vertices
		//norBuf = shapes[0].mesh.normals;    //normals
		//texBuf = shapes[0].mesh.texcoords;
		eleBuf = shapes[0].mesh.indices;    //faces

      //init all normals to 0
      for (size_t v = 0; v < posBuf.size(); v++) {
         norBuf.push_back(0);
      }

      //fill in normals
      float idx1, idx2, idx3;  //index of vertieces
      myVec a, b, c;  //vertices
      myVec cross, vec1, vec2;

      printf("NumFace = %d\n", eleBuf.size() / 3);

      for (size_t f = 0; f < eleBuf.size()/3; f++) {
         idx1 = eleBuf[3 * f + 0];  //getting the vertices of face f
         idx2 = eleBuf[3 * f + 1];
         idx3 = eleBuf[3 * f + 2];
